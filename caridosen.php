...
<fieldset>
   ...
   if($cari) {
    ...
    $sql = mysqli_query($link,"SELECT * FROM dosen WHERE nama like '%$input_cari%' OR nip like '%$input_cari%' OR keahlian like '%$input_cari%'") or die (mysql_error());   
    } else {
    $sql = mysqli_query($link,"SELECT * FROM dosen") or die (mysql_error());
    }
    } else {
    $sql = mysqli_query($link,"SELECT * FROM dosen") or die (mysql_error());
    }
   ...
   if($cek < 1) {
    ?>
        <tr>
            ...
            <td colspan="7" align="center" style="padding:10px;"><h4>Data Tidak Ditemukan</h4></td>
        </tr>
        <?php
   } else {
